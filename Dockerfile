...
# build-base is required for @swc/core native binaries on Alpine Linux
RUN apk add --no-cache libc6-compat build-base python3-dev
WORKDIR /app
...
# Leverage BuildKit cache with separate tmp directory for npm cache
# to avoid issues with @swc/core native binaries on Alpine Linux
RUN --mount=type=cache,target=/tmp/npm-cache \
    npm ci --install-links --include=peer --cache /tmp/npm-cache
